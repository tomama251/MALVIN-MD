...
const { lords } = require(__dirname + "/../lordmalvin/lords");
const { format } = require(__dirname + "/../lordmalvin/mesfonctions");
const os = require("os");
...
lords({ nomCom: "scan", categorie: "General" }, async (dest, zk, commandeOptions) => {
    let { ms, repondre ,prefixe,nomAuteurMessage,mybotpic} = commandeOptions;
    let { cm } = require(__dirname + "/../lordmalvin//lords");
    var coms = {};
    var mode = "public";
...
`;
   ...
   if (lien.match(/\.(mp4|gif)$/i)) {
    try {
        zk.sendMessage(dest, { video: { url: lien }, caption:infoMsg + menuMsg, footer: "Je suis *𝙼𝙰𝙻𝚅𝙸𝙽*, déveloper ᴹᴬᴸᵛᴵᴺ Tech" , gifPlayback : true }, { quoted: ms });
    }
    catch (e) {
...
// Vérification pour .jpeg ou .png
    ...
    try {
        zk.sendMessage(dest, { image: { url: lien }, caption:infoMsg + menuMsg, footer: "Je suis *𝙼𝙰𝙻𝚅𝙸𝙽*, déveloper 𝙼𝙰𝙻𝚅𝙸𝙽 Tech" }, { quoted: ms });
    }
    catch (e) {
        console.log("🥵🥵 Menu erreur " + e);
        repondre("🥵🥵 Menu erreur " + e);
