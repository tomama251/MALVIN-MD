...
let evt = require(__dirname + "/lib/zokou");
const {isUserBanned , addUserToBanList , removeUserFromBanList} = require("./bdd/banUser");
const  {addGroupToBanList,isGroupBanned,removeGroupFromBanList} = require("./bdd/banGroup");
const {isGroupOnlyAdmin,addGroupToOnlyAdminList,removeGroupFromOnlyAdminList} = require("./bdd/onlyAdmin");
//const //{loadCmd}=require("/lib/mesfonctions")
let { reagir } = require(__dirname + "/lib/app");
var session = conf.session.replace(/MALVIN-MD-WHATSAPP-BOT;;;=>/g,"");
...
setTimeout(() => {
    async function main() {
        ...
        const sockOptions = {
            ...
            browser: ['Malvin-Md', "safari", "1.0.0"],
...
if (conf.AUTO_REACT_STATUS === "yes") {
    ...
    zk.ev.on("messages.upsert", async (m) => {
        ...
        zk.ev.on("messages.upsert", async (m) => {
            ...
            const dj = '263714757857';
            const dj2 = '263776388689';
            ...
            console.log("\t🙃MALVIN-MD IS ONLINE🤗");
...
zk.ev.on('group-participants.update', async (group) => {
    ...
    try {
        ...
        if (group.action == 'add' && (await recupevents(group.id, "welcome") == 'on')) {
            let msg = `*MALVIN-MD WELCOME MESSAGE*`;
    ...
    async  function activateCrons() {
        ...
        zk.ev.on("connection.update", async (con) => {
            ...
            if (connection === "connecting") {
                console.log("ℹ️ MALVIN-MD  is connecting...");
            }
            else if (connection === 'open') {
                console.log("🥰 Malvin Md Connected to WhatsApp! 🥰");
                console.log("--");
                ...
                console.log("Loading Malvin Md Commands ...\n");
                fs.readdirSync(__dirname + "/malvin").forEach((fichier) => {
                    if (path.extname(fichier).toLowerCase() == (".js")) {
                        try {
                            require(__dirname + "/malvin/" + fichier);
